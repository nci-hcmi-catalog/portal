...
const getClient = (pm2config) => {
  ...
  const region =
    process.env.SEARCH_ENGINE_AUTH_REGION || pm2config.SEARCH_ENGINE_AUTH_REGION || 'us-east-1';
  const service =
    process.env.SEARCH_ENGINE_AUTH_SERVICE || pm2config.SEARCH_ENGINE_AUTH_SERVICE || 'es';
